...
function GooFlow(bgDiv, property) {
	...
	this.$el = bgDiv; //最父框架的DIV
	this.$el.addClass("GooFlow");
	if (GooFlow.prototype.color.font) {
		this.$el.css("color", GooFlow.prototype.color.font);
	}
	...
	if (property.haveTool) {
		this.$el.append("<div class='GooFlow_tool'" + (property.haveHead ? "" : " style='margin-top:3px'") + "><div style='height:" + (height - headHeight - (property.haveHead ? 7 : 10)) + "px' class='GooFlow_tool_div'></div></div>");
		this.$tool = this.$el.find(".GooFlow_tool div");
		//未加代码：加入绘图工具按钮
		this.$tool.append(
	...
	this.$el.append("<div class='GooFlow_work' style='width:" + (width) + "px;height:" + (height) + "px;" + (property.haveHead ? "" : "margin-top:3px") + "'></div>");
